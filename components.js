...
const PrintableOrder = ({ order, shopInfo }) => {
    ...
    const bill = totalPrice <= 500 ? 500 : (totalPrice <= 1000 ? 1000 : Math.ceil(totalPrice / 1000) * 1000);
    ...
    return React.createElement('div', { style: s.container },
        ...
        React.createElement('div', { style: s.footer },
            ...
            React.createElement('div', null, `小計: $${Math.round(order.subtotal || totalPrice / 1.05)}`),
            React.createElement('div', null, `税金5%: +$${Math.round(order.taxAmount !== undefined ? order.taxAmount : (totalPrice - Math.round(totalPrice / 1.05)))}`),
...
const DashboardTab = ({ orders = [], stats = null }) => {
    ...
    const processedData = React.useMemo(() => {
        ...
        const currentRev = currentValid.reduce((s, o) => s + (o.subtotal !== undefined ? o.subtotal : Math.round((o.totalPrice || 0) / 1.05)), 0);
        ...
        const prevRev = prevValid.reduce((s, o) => s + (o.subtotal !== undefined ? o.subtotal : Math.round((o.totalPrice || 0) / 1.05)), 0);
        ...
        const hourlyDistribution = new Array(24).fill(0).map((_, h) => {
            ...
            // ★ C-02 修正：小時統計也用稅前金額
            return { h, count: hOrders.length, rev: hOrders.reduce((s, o) => s + (o.subtotal !== undefined ? o.subtotal : Math.round((o.totalPrice || 0) / 1.05)), 0) };
        });
    ...
    const exportReport = () => {
        ...
        orders.forEach(o => {
            const preTax = o.subtotal !== undefined ? o.subtotal : Math.round((o.totalPrice || 0) / 1.05);
                    ...
                    React.createElement('div', { className: "bg-slate-900 p-8 rounded-[2.5rem] text-white flex justify-between items-center shadow-xl" },
                        React.createElement('div', { className: "space-y-1" },
                            ...
                            React.createElement('div', { className: "flex gap-6 items-baseline" },
                                ...
                                React.createElement('span', { className: "text-2xl font-black font-mono text-slate-300" }, `+$${Math.round(inspectingOrder.taxAmount || Math.round((inspectingOrder.subtotal || inspectingOrder.totalPrice) * 0.05)).toLocaleString()}`)
                            ),
...
const MaintenanceTab = ({
    ...
    const handleForceOverwrite = async () => {
        ...
        try {
            ...
            await apiService.saveMenuConfig([], [], {}, {});
            await apiService.saveSettings({ kiosk_config: {} });
            ...
            // ★ 修正：使用 Promise.all 確保所有 IDB 寫入完成後才繼續（避免 Race Condition）
            await Promise.all([
                apiService.saveMenuConfig(
                    snapshot.data.menu || [],
                    snapshot.data.addons || [],
                    snapshot.data.options || {},
                    snapshot.data.groupMeta || {}  // ★ 修正：舊快照無 groupMeta 時 fallback 空物件
                ),
                apiService.saveSettings({ kiosk_config: snapshot.data.sysConfig })
            ]);
            ...
            setTimeout(() => {
                alert(`✅ 強制覆寫完成！\n已導入 ${snapshot.data.menu.length} 個菜單項目\n系統即將重新載入...`);
                window.location.reload();
            }, 1500);
...
const ItemModal = ({ selectedItem, editingItem, addons, options, featureToggles, onClose, onConfirmSelection, t }) => {
    ...
    const subtotal = React.useMemo(() => {
        // ★ 修正：base 已乘 quantity；選項費用也需乘 quantity（每份都有該選項）
        ...
        Object.values(selections).forEach(group => {
            if (Array.isArray(group)) {
                group.forEach(opt => {
                    if (opt.price) base += opt.price * (opt.quantity || 1) * quantity;
                });
            }
        });

        // 加購費用：加購是全單共用（不隨份數倍增），單獨加總
        let addonPrice = selectedAddons.reduce((sum, a) => sum + (a.price * (a.quantity || 1)), 0);
        
        return base + addonPrice;
    }, [item.price, quantity, selections, selectedAddons]);

    // BUG-FIX-2: 顯示用小計四捨五入，避免浮點誤差
    const subtotalDisplay = Math.round(subtotal);
...
const PasswordDialog = ({ isOpen, onClose, onSuccess, config }) => {
    ...
    const [livePassword, setLivePassword] = React.useState('admin123');

    // 每次開啟時即時從 IDB 讀取最新密碼（避免 config 快照過舊），並重置錯誤狀態
    React.useEffect(() => {
        if (!isOpen) return;
        // ★ 修正：重新開啟時清除上次的錯誤訊息和輸入內容
        setPassword('');
        setError('');
        apiService.getSettings().then(s => {
            const pw = s && s.kiosk_config && s.kiosk_config.content && s.kiosk_config.content.adminPassword;
            setLivePassword(pw || config?.content?.adminPassword || 'admin123');
        }).catch(() => {
            setLivePassword(config?.content?.adminPassword || 'admin123');
        });
    }, [isOpen]);
    
    const handleSubmit = () => {
        const storedPassword = livePassword;
    ...
    return React.createElement('div', {
        ...
        React.createElement('div', {
            className: "bg-white rounded-3xl p-5 max-w-md w-full mx-4 shadow-2xl",
            onClick: (e) => e.stopPropagation()
        },
            React.createElement('div', { className: "text-center mb-8" },
                React.createElement('div', { className: "text-6xl mb-4" }, '🔒'),
                React.createElement('h2', { className: "text-3xl font-black text-slate-800 mb-2" }, '管理員登入'),
                React.createElement('p', { className: "text-slate-500" }, '請輸入管理員密碼')
            ),
            React.createElement('div', { className: "space-y-4" },
                React.createElement('input', {
                    type: "password",
                    ...
                    placeholder: "請輸入密碼",
                    className: "w-full p-4 border-2 border-slate-300 rounded-2xl text-xl font-bold text-center focus:border-indigo-500 focus:outline-none",
                    autoFocus: true
                }),
                error && React.createElement('p', { className: "text-rose-500 text-center font-bold" }, error),
                React.createElement('div', { className: "flex gap-3" },
                    React.createElement('button', {
                        onClick: onClose,
                        className: "flex-1 py-4 bg-slate-200 text-slate-700 rounded-2xl font-bold text-lg hover:bg-slate-300 transition-colors"
                    }, '取消'),
                    React.createElement('button', {
                        onClick: handleSubmit,
                        className: "flex-1 py-4 bg-indigo-600 text-white rounded-2xl font-bold text-lg hover:bg-indigo-700 transition-colors"
                    }, '登入')
                ),
                React.createElement('p', { className: "text-center text-sm text-slate-400 mt-4" }, 
                    '若忘記密碼請聯繫管理員',
...
const Cart = ({ isOpen, onClose, cartItems, onUpdateQuantity, onRemoveItem, onSubmitAndPrint, isSubmitting, t, onContinueOrdering }) => {
    const totals = React.useMemo(() => {
        // BUG-FIX-2: 所有金額統一四捨五入，防止浮點誤差影響顯示與儲存
        const subtotal = Math.round(cartItems.reduce((acc, item) => acc + (Number(item.totalPrice) || 0), 0));
        const taxAmount = Math.round(subtotal * 0.05);
        return { subtotal, taxAmount, finalTotal: subtotal + taxAmount };
    }, [cartItems]);
                ...
                React.createElement('div', { className: "w-full" },
                    ...
                    activeTab === 'maintenance' && React.createElement(MaintenanceTab, {
                        ...
                        onRestoreFromSnapshot: async (snap) => {
                            // ★ 修正：舊版快照可能缺少部分欄位，全部加 || 安全防護
                            // ★ 修正：sysConfig 合併 DEFAULT_SYS_CONFIG，防止欄位缺失造成白畫面
                            const rawSysConfig = snap.data.sysConfig || {};
                            const safeSysConfig = {
                                ...DEFAULT_SYS_CONFIG,
                                ...rawSysConfig,
                                featureToggles: { ...(DEFAULT_SYS_CONFIG.featureToggles || {}), ...(rawSysConfig.featureToggles || {}) },
                            ...
                            // ★ 修正：使用 Promise.all 確保所有 IDB 寫入完成（避免 Race Condition）
                            await Promise.all([
                                apiService.saveMenuConfig(
                                    snap.data.menu || [],
                                    snap.data.addons || [],
                                ...
                                apiService.saveSettings({ kiosk_config: safeSysConfig })
                            ]);
                            await loadData();
                            triggerSyncPulse('restore');
                            showToast('系統還原中，即將重新載入...', 'info', 1500);
...
const App = () => {
    ...
    const handleItemConfirm = (item, quantity, customizations, category) => {
        const cartId = `cart_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        
        // BUG-FIX-2: 計算單位價格時統一四捨五入，防止浮點誤差在加總時累積
        // ★ 修正：選項費用 = 每份選項費用（不乘 quantity，因為 unitPriceWithExtras 代表每份）
        const optionsTotal = Object.values(customizations.dynamicSelections || {}).reduce((sum, opts) => 
            sum + opts.reduce((s, opt) => s + ((opt.price || 0) * (opt.quantity || 1)), 0), 0);
        // ★ 修正：加購是全單共用（不隨份數倍增），與 ItemModal subtotal 計算邏輯一致
        const addonsTotal = (customizations.addons || []).reduce((sum, addon) => 
            sum + ((addon.price || 0) * (addon.quantity || 1)), 0);
        // unitPriceWithExtras = 基本單價 + 每份選項費用（不含加購）
        const unitPriceWithExtras = Math.round(item.price + optionsTotal);
        // totalPrice = 單價 × 份數 + 加購（全單共用）
    ...
    const handleUpdateQuantity = (cartId, newQty) => {
        ...
        setCartItems(cartItems.map(item => {
            ...
            // ★ 修正：totalPrice = 單價×份數 + 加購費用（加購不隨份數倍增）
            const addonsFlat = (item.addons || []).reduce((s, a) => s + ((a.price || 0) * (a.quantity || 1)), 0);
            return { ...item, quantity: newQty, totalPrice: Math.round(item.unitPriceWithExtras * newQty + addonsFlat) };
        }));
    };
    ...
    const handleSubmitOrder = async (orderData) => {
        try {
            ...
            if (result.success) {
                ...
                // 同步訂單資料到 invoice_server（讓發票管理可對帳）
                fetch('http://127.0.0.1:8765/sync_order', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        id:          newOrder.id,
                        orderType:   newOrder.orderType || '',
                        guestCount:  newOrder.guestCount || 1,
                        totalPrice:  newOrder.totalPrice || 0,
                        subtotal:    newOrder.subtotal || 0,
                        taxAmount:   newOrder.taxAmount || 0,
                        totalWithTax:newOrder.totalWithTax || newOrder.totalPrice || 0,
                        status:      newOrder.status || 'completed',
                        createdAt:   newOrder.createdAt || new Date().toISOString(),
                        items:       (newOrder.items || []).map(function(it){
                            return { name: it.name, quantity: it.quantity, totalPrice: it.totalPrice };
                        })
                    })
                }).catch(function(){});  // 靜默失敗，不影響主流程

                // 印二聯式發票到 WP-550（每張訂單只印一次）
                // ★ 修正：改用 sessionStorage 防重複，避免 window.print() / kiosk 模式清空 window 物件
                const invoiceOrderId = newOrder.id;
                const _ssKey = 'printed_inv_' + invoiceOrderId;
                if (!sessionStorage.getItem(_ssKey)) {
                    sessionStorage.setItem(_ssKey, '1');
                    setTimeout(() => {
                        const invoiceTotal = Math.round(newOrder.totalWithTax || newOrder.totalPrice || 0);
                        fetch('http://127.0.0.1:8765/print_invoice', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ total: invoiceTotal, buyer: '', order_id: invoiceOrderId })
                        })
                        .then(function(res) { return res.json(); })
                        .then(function(data) {
                            if (data.ok) {
                                showToast('✅ 發票 ' + data.invoice + ' 印出！', 'success', 3000);
                            } else {
                                showToast('⚠️ 發票失敗：' + (data.error || '未知錯誤'), 'error', 5000);
                            }
                        })
                        .catch(function(err) {
                            showToast('⚠️ 發票印表機未連線，請確認 invoice_server.py 已執行', 'error', 6000);
                        });
                    }, 400);
                }
            } else {
